...
export const Route = createFileRoute("/admin/analytics")({
  head: () => ({
    meta: [{ title: "Analytics — LegitBodyFix Admin" }, { name: "robots", content: "noindex" }],
  }),
...
function isVerificationEvent(view: View) {
  return (
    view.path === "/analytics-verification" ||
    view.utm_source === "codex-verification" ||
    view.utm_campaign?.startsWith("analytics-check-")
  );
}
...
function groupRecentSessions(views: View[]): RecentSession[] {
  ...
  grouped.forEach((session) => {
    ...
    visitsByVisitor.set(session.visitorId, [
      ...(visitsByVisitor.get(session.visitorId) || []),
      session,
    ]);
  });
  visitsByVisitor.forEach((visitorSessions) => {
    ...
    visitorSessions.forEach((session, index) => {
      session.visitorSessionNumber = index + 1;
    });
  });
...
function AnalyticsPage() {
  ...
  const report = useMemo(() => {
    ...
    const previousAcquisitionRate = previousSessions
      ? (previousAcquired / previousSessions) * 100
      : 0;
  ...
  const measuredSessions = recentSessions.filter(
    (session) => session.observedDurationSeconds !== null,
  );
  const averageDuration = measuredSessions.length
    ? measuredSessions.reduce((sum, session) => sum + (session.observedDurationSeconds ?? 0), 0) /
      measuredSessions.length
    : null;
  const collectionActive = latestView
    ? Date.now() - new Date(latestView).getTime() < 86_400_000
    : false;
  const timeZone = Intl.DateTimeFormat().resolvedOptions().timeZone;
  const bestHour = report.hourly.reduce(
  ...
  const statCards = [
    {
      ...
      value: formatSessionDuration(averageDuration),
      note: measuredSessions.length + " of " + recentSessions.length + " sessions measurable",
      delta: undefined,
    },
    {
      label: "Page views",
      value: report.views.length.toLocaleString(),
      note: `Last ${range} days`,
      delta: report.changes.views,
    },
    {
      label: "Sessions",
      value: report.sessions.toLocaleString(),
      note: "Approximate visits",
      delta: report.changes.sessions,
    },
    {
      label: "Anonymous visitors",
      value: report.uniqueVisitors ? report.uniqueVisitors.toLocaleString() : "—",
      note: "Recognized browsers",
      delta: undefined,
    },
    {
      label: "Pages / session",
      value: report.sessions ? report.pagesPerSession.toFixed(1) : "—",
      note: "Browsing depth",
      delta: report.changes.depth,
    },
    {
      label: "Acquired visits",
      value: report.sessions ? `${Math.round(report.acquisitionRate)}%` : "—",
      note: "Referrer or UTM found",
      delta: report.changes.acquisition,
    },
  ];
  ...
  return (
    <div className="mx-auto max-w-7xl px-5 py-6 lg:px-8">
      <PageHead
        ...
        meta="Understand traffic, acquisition, and content demand"
        actions={
        ...
        <div className="flex flex-wrap gap-2" aria-label="Date range">
          {([7, 30, 90] as Range[]).map((days) => (
            <button
              ...
              className={`min-h-10 border px-4 font-mono text-xs font-bold uppercase tracking-wider ${range === days ? "border-ink bg-ink text-ink-foreground" : "border-border bg-background"}`}
            >
              {days} days
            </button>
          ))}
        ...
        <>
          ...
          >
            <div className="flex items-center gap-3">
              <span
                className={`h-2.5 w-2.5 rounded-full ${collectionActive ? "bg-accent" : "bg-muted-foreground"}`}
              />
              <div>
                ...
                <p className="mt-1 text-sm font-bold">
                  {collectionActive
                    ...
                    : latestView
                      ...
                      : "Waiting for the first real visit"}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <Panel className="p-5">
                <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between">
                  <h2 className="text-lg font-extrabold">Visits by hour</h2>
              ...
              <Panel className="p-5">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <h2 className="text-lg font-extrabold">
                    {acquisitionMode === "source" ? "Traffic sources" : "Campaigns"}
                  </h2>
                  <ModeToggle value={acquisitionMode} onChange={setAcquisitionMode} />
                </div>
                <RankList
                  ...
                  empty={
                    acquisitionMode === "source"
                      ? "Direct and referred traffic will appear here."
                      : "Add UTM campaign tags to marketing links to measure them here."
                  }
                />
              </Panel>
          ...
          <div className="mt-4">
            <RankPanel
              title="Top pages"
              rows={report.pages}
              empty="Page activity will appear after visitors browse the site."
              formatLabel={(label) => (label === "/" ? "Homepage" : label)}
            />
          </div>
          ...
          <Panel className="mt-4 overflow-hidden">
            <div className="flex flex-col gap-1 border-b border-border px-5 py-4 sm:flex-row sm:items-baseline sm:justify-between">
              <div>
                <h2 className="text-lg font-extrabold">Recent sessions</h2>
                <p className="mt-1 text-xs text-muted-foreground">
                  Open a session to see its page-by-page journey. A random browser ID groups repeat
                  visits without using names, email addresses, or precise location.
                </p>
              </div>
              <span className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground">
            ...
            {recentSessions.length > 0 && (
              <div className="flex flex-col gap-3 border-t border-border px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                <p className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground">
                  Showing {Math.min(visibleSessionCount, recentSessions.length)} of{" "}
                ...
                {visibleSessionCount < recentSessions.length ? (
                  <button
                    ...
                    className="min-h-10 border border-border bg-background px-5 text-xs font-bold hover:border-foreground"
                  >
                    Load more
                  </button>
                ) : null}
          ...
          <p className="mt-5 text-xs leading-relaxed text-muted-foreground">
            Privacy note: analytics stores random browser and per-tab session IDs, page path, device
            category, referrer domain, UTM tags, and approximate administrative areas. Hosting-edge
            coordinates are used only to derive city, county, district, town, or township labels and
...
function SessionJourney({ session }: { session: RecentSession }) {
  const visitorLabel = session.visitorId
    ? `Visitor ${session.visitorId.slice(0, 6).toUpperCase()}`
    : "Legacy session";
  const visitLabel =
    session.visitorSessionNumber && session.visitorSessionNumber > 1
    ...
    <details className="group">
      <summary className="grid cursor-pointer list-none gap-2 px-5 py-4 text-sm hover:bg-secondary/35 sm:grid-cols-[minmax(0,1fr)_auto_auto_auto_auto] sm:items-center sm:gap-5">
        ...
        <div className="text-xs text-muted-foreground">
          <p
            className="font-bold"
            title="Estimated time from first to last page view. Time on the final page is not measured."
          >
        ...
        <time
          ...
          className="font-mono text-[11px] text-muted-foreground sm:text-right"
        >
          {new Intl.DateTimeFormat(undefined, { dateStyle: "medium", timeStyle: "short" }).format(
      ...
      <div className="border-t border-border bg-secondary/20 px-5 py-4">
        <div className="mb-4 flex flex-wrap gap-x-5 gap-y-1 font-mono text-[10px] uppercase tracking-wider text-muted-foreground">
          <span>{visitorLabel}</span>
          <span>
            {session.networkHash
              ? `Network ${session.networkHash.slice(0, 8).toUpperCase()}`
              : "Network unavailable"}
          </span>
          <span>
            {formatLocation(
        ...
        <ol className="space-y-0">
          {session.journey.map((step, index) => (
            <li
              key={`${step.visitedAt}-${index}`}
              className="grid grid-cols-[1.5rem_minmax(0,1fr)_auto] gap-3 text-sm"
            >
              <div className="flex flex-col items-center">
                <span className="grid h-6 w-6 place-items-center rounded-full bg-ink font-mono text-[10px] text-ink-foreground">
                  {index + 1}
                </span>
              ...
              <div className="min-w-0 pt-0.5">
                <p className="truncate font-medium">{step.path === "/" ? "Homepage" : step.path}</p>
                {step.secondsFromPrevious !== null ? (
                  <p className="mt-0.5 text-[11px] text-muted-foreground">
                    {formatSessionDuration(step.secondsFromPrevious)} after previous page
                  </p>
                ) : null}
              </div>
              <time
                dateTime={step.visitedAt}
                className="pt-0.5 font-mono text-[10px] text-muted-foreground"
              >
                {new Intl.DateTimeFormat(undefined, {
                  hour: "2-digit",
                  minute: "2-digit",
                  second: "2-digit",
                }).format(new Date(step.visitedAt))}
              </time>
            </li>
          ))}
        </ol>
        {session.journey.length === 1 ? (
          <p className="mt-3 text-xs text-muted-foreground">
            No next page was recorded, so time on this page cannot be estimated.
          </p>
        ) : null}
      </div>
    </details>
...
function formatLocation(
  ...
  const region =
    countryCode === "KR" && regionCode ? KOREA_REGIONS[regionCode] || regionCode : regionCode;
  return [country, region, administrativeArea || city].filter(Boolean).join(" · ");
}
...
function Delta({ value }: { value: number | null | undefined }) {
  ...
  return (
    <span
      className={`inline-flex items-center text-[10px] font-bold ${positive ? "text-emerald-700" : negative ? "text-destructive" : "text-muted-foreground"}`}
    >
      {positive ? (
...
function ModeToggle({
  ...
  onChange: (value: AcquisitionMode) => void;
}) {
  return (
    <div className="inline-flex self-start rounded-full border border-border bg-background p-1 text-xs font-bold">
      <button
        type="button"
        onClick={() => onChange("source")}
        className={`rounded-full px-3 py-2 transition-colors ${value === "source" ? "bg-ink text-ink-foreground" : "text-muted-foreground hover:text-foreground"}`}
      >
        Sources
      </button>
      <button
        ...
        className={`rounded-full px-3 py-2 transition-colors ${value === "campaign" ? "bg-ink text-ink-foreground" : "text-muted-foreground hover:text-foreground"}`}
      >
        Campaigns
      </button>
    </div>
...
function AcquisitionTrend({
  ...
  if (!series.length)
    return (
      <div className="mt-6 grid h-44 place-items-center border border-dashed border-border text-center text-sm text-muted-foreground">
        No {mode === "source" ? "source" : "campaign"} trend data yet.
      </div>
    );

  return (
    <>
      <div className="relative mt-6 h-44 w-full" aria-label={`${mode} traffic trend`}>
        <svg
        ...
        >
          {[0, 1, 2].map((lineIndex) => (
            <line
              ...
              x1={inset}
              x2={width - inset}
              y1={inset + (lineIndex * (height - inset * 2)) / 2}
              y2={inset + (lineIndex * (height - inset * 2)) / 2}
                ...
                {points.map(({ count, date, x, y }) => (
                  <g key={date}>
                    ...
                    <circle
                      ...
                      tabIndex={0}
                      onPointerEnter={() =>
                        setActivePoint({ label: item.label, date, count, x, y, color })
                      }
                      onPointerDown={() =>
                        setActivePoint({ label: item.label, date, count, x, y, color })
                      }
...
function HourlyLine({ rows }: { rows: readonly (readonly [number, number])[] }) {
  ...
  const points = rows.map(([hour, count], index) => ({
    hour,
    count,
    x: inset + (index / Math.max(1, rows.length - 1)) * (width - inset * 2),
    y: height - inset - (count / max) * (height - inset * 2),
  }));
  return (
    <div className="relative mt-6 h-40 w-full" aria-label="Visits by local hour">
      <svg
        viewBox={`0 0 ${width} ${height}`}
        preserveAspectRatio="none"
        className="h-full w-full overflow-visible"
      ...
      >
        ...
        <polyline
          ...
          strokeLinecap="round"
        />
        {points.map(({ hour, count, x, y }) => (
          <g key={hour}>
            <circle
              cx={x}
              cy={y}
              r={count > 0 ? "4" : "2"}
              className={count > 0 ? "fill-card stroke-ink" : "fill-accent"}
            ...
            <circle
              ...
              className="cursor-crosshair"
              tabIndex={0}
              onPointerEnter={() => setActivePoint({ hour, count, x, y })}
              onPointerDown={() => setActivePoint({ hour, count, x, y })}
              onFocus={() => setActivePoint({ hour, count, x, y })}
              onBlur={() => setActivePoint(null)}
            >
              <title>
...
function ChartTooltip({
  ...
  return (
    <div
      className="pointer-events-none absolute z-10 min-w-max border border-ink bg-ink px-3 py-2 text-left text-ink-foreground shadow-lg"
      style={{ left: `${x}%`, top: `${y}%`, transform: `${horizontal} ${vertical}` }}
      role="status"
    >
...
function DevicePanel({ rows }: { rows: [string, number][] }) {
  ...
  return (
    <Panel className="p-5">
      <h2 className="text-lg font-extrabold">Device mix</h2>
      {rows.length ? (
        <div className="mt-5 flex items-center gap-6">
          <div
            className="relative h-28 w-28 shrink-0 rounded-full"
            style={{
              background: `conic-gradient(hsl(var(--accent)) 0deg ${angle}deg, hsl(var(--ink)) ${angle}deg 360deg)`,
            }}
          >
            ...
            {rows.map(([label, value], index) => (
              <div key={label} className="flex items-center justify-between gap-3 text-sm">
                <span className="flex items-center gap-2 capitalize">
                  <span className={`h-2.5 w-2.5 ${index === 0 ? "bg-accent" : "bg-ink"}`} />
                  {label}
                </span>
        ...
        <p className="py-8 text-sm text-muted-foreground">
          Device data will appear after the first visit.
        </p>
      )}
    </Panel>
  );
  ...
  return (
    <div className="mt-4 divide-y divide-border">
      {rows.slice(0, 8).map(([label, value], index) => (
        <div
          key={label}
          className={`grid grid-cols-[1.5rem_minmax(0,1fr)_auto] items-center gap-2 py-3 text-sm ${index === 0 ? "text-emerald-700" : ""}`}
        >
      ...
      {!rows.length && <p className="py-10 text-sm text-muted-foreground">{empty}</p>}
    </div>
  );
}
...
function RankPanel({
  ...
  return (
    <Panel className="p-5">
      ...
      <div className={numbered ? "mt-4 divide-y divide-border" : "mt-5 space-y-5"}>
        {rows.slice(0, 8).map(([label, value], index) =>
          numbered ? (
            <div
              key={label}
            ...
            <div key={label}>
              ...
              <div className="h-2 bg-secondary">
                <div className="h-full bg-ink" style={{ width: `${(value / max) * 100}%` }} />
              </div>
            </div>
          ),
        )}
