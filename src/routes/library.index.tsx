...
function LibraryIndex() {
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-9 sm:px-5 sm:py-12 lg:px-8 lg:py-20">
      ...
      <div className="mt-9 grid gap-px bg-border sm:mt-12 sm:grid-cols-2 lg:grid-cols-3">{programs.map((program) => <Link key={program.id} to="/library/$programSlug" params={{ programSlug: program.slug }} className="group bg-card p-5 transition-colors hover:bg-secondary sm:p-6"><ProgramArtwork program={program} /><p className="mt-4 font-mono text-[10px] uppercase tracking-[0.14em] text-muted-foreground sm:mt-5">{program.duration_label || program.format || "On-demand program"}</p><h2 className="mt-2 text-xl font-extrabold tracking-tight sm:text-2xl">{program.name}</h2><p className="mt-2 line-clamp-3 text-sm leading-6 text-muted-foreground">{program.outcome}</p><span className="mt-5 inline-flex min-h-11 items-center gap-2 text-sm font-bold sm:mt-6">Open program <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></span></Link>)}</div>
    </main>
  );
}
