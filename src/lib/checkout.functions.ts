...
export const createProgramCheckout = createServerFn({ method: "POST" })
  .validator((input) => z.object({ programSlug: z.string().regex(/^[a-z0-9-]+$/), returnOrigin: z.string().url() }).parse(input))
  .handler(async ({ data }) => {
    const origin = allowedOrigin(data.returnOrigin);
    const { data: program, error } = await serverClient().from("programs").select("id,slug,stripe_price_id,stripe_price_lookup_key,published").eq("slug", data.programSlug).eq("published", true).single();
    ...
    if (!priceId && program.stripe_price_lookup_key) {
      const params = new URLSearchParams({ active: "true", limit: "1", "lookup_keys[]": program.stripe_price_lookup_key });
      const response = await fetch(`${apiBase}/v1/prices?${params}`, { headers: stripeHeaders(stripeKey, lovableKey) });
      const payload = await response.json() as { data?: { id: string }[]; error?: { message?: string } };
      if (!response.ok) throw new Error(payload.error?.message || "Could not resolve the Stripe price.");
    ...
    const response = await fetch(`${apiBase}/v1/checkout/sessions`, {
      method: "POST",
      headers: { ...stripeHeaders(stripeKey, lovableKey), "Content-Type": "application/x-www-form-urlencoded" },
      body,
    });
...
export const getCheckoutStatus = createServerFn({ method: "GET" })
  .validator((input) => z.object({ sessionId: z.string().regex(/^cs_(?:test_|live_)[A-Za-z0-9]+$/) }).parse(input))
  .handler(async ({ data }) => {
    const { stripeKey, lovableKey, apiBase } = stripeConfig();
    const response = await fetch(`${apiBase}/v1/checkout/sessions/${encodeURIComponent(data.sessionId)}`, {
...
function allowedOrigin(value: string) {
  ...
  const allowed = url.protocol === "https:" && (
    url.hostname === "legitbodyfix.com" || url.hostname === "www.legitbodyfix.com" ||
