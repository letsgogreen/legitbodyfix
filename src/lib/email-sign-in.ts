...
export async function requestEmailLink(email: string, origin: string, send: EmailLinkSender): Promise<string> {
  const address = email.trim();
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(address)) throw new Error("Enter a valid email address.");
  try {
    const { error } = await send({ email: address, options: { emailRedirectTo: `${origin}/library` } });
