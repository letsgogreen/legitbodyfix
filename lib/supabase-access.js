...
function AccessError(code, statusCode, details) {
...
async function responseFailureDetails(response) {
    ...
    if (body && typeof body === "object") {
      ["code", "message", "hint"].forEach(function (name) {
        if (typeof body[name] === "string" && body[name]) {
          details[name] = body[name].slice(0, 300);
        }
      });
    }
  } catch (error) {
    // The HTTP status is still enough to diagnose a non-JSON provider response.
  }
  return details;
}
...
async function listEntitlements(config, email, fetchImplementation) {
  ...
  if (!response.ok) {
    throw new AccessError(
      "supabase_entitlement_lookup_failed",
      502,
      await responseFailureDetails(response)
    );
  }
...
function purchaseEndpoint(config, email) {
  var endpoint = new URL(config.url + "/rest/v1/payment_orders");
  endpoint.searchParams.set("select", "id,provider_order_id,program_id,amount,currency,status,paid_at,created_at");
  endpoint.searchParams.set("buyer_email", "eq." + email);
  endpoint.searchParams.set("status", "eq.completed");
  endpoint.searchParams.set("order", "paid_at.desc");
  return endpoint.toString();
}

async function listPurchases(config, email, fetchImplementation) {
  var fetcher = fetchImplementation || globalThis.fetch;
  if (typeof fetcher !== "function") throw new AccessError("access_service_unavailable", 503);

  var response;
  try {
    response = await fetcher(purchaseEndpoint(config, email), {
      headers: {
        "apikey": config.secretKey,
        "Accept": "application/json"
      }
  ...
  if (!response.ok) {
    throw new AccessError("supabase_purchase_lookup_failed", 502, await responseFailureDetails(response));
  }
  var rows = await parseJson(response);
  if (!Array.isArray(rows)) throw new AccessError("supabase_response_invalid", 502);
  ...
  return rows.map(function (row) {
    return {
      ...
      orderId: typeof row.provider_order_id === "string" ? row.provider_order_id : "",
      programId: typeof row.program_id === "string" ? row.program_id : "",
      amount: Number(row.amount) || null,
      currency: typeof row.currency === "string" ? row.currency : "USD",
      status: "completed",
      paidAt: typeof row.paid_at === "string" ? row.paid_at : (typeof row.created_at === "string" ? row.created_at : null)
    };
  });
}
