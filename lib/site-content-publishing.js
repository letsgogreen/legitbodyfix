...
var CONTENT_PATH = "public/ver1-snapshot/assets/data/site-content.json";
var CORE_SECTIONS = ["hero", "method", "library", "knowledge", "standard", "pricing"];
...
function validateSiteContent(input) {
  ...
  var knowledge = object(root.knowledge, "knowledge", errors);
  ...
  var steps = Array.isArray(method.steps) && method.steps.length === 4 ? method.steps : [];
  if (steps.length !== 4) errors.push("method.steps must contain exactly 4 items.");
  ...
  var content = {
    ...
    method: {
      ...
      steps: Array.from({ length: 4 }, function (_, index) {
    ...
    knowledge: {
      label: plainText(knowledge.label, "knowledge.label", 80, errors, true),
      titleLines: strings(knowledge.titleLines, "knowledge.titleLines", 2, 40, errors),
      intro: plainText(knowledge.intro, "knowledge.intro", 320, errors, true),
      linkLabel: plainText(knowledge.linkLabel, "knowledge.linkLabel", 40, errors, true),
      linkHref: safeLink(knowledge.linkHref, "knowledge.linkHref", errors, true)
    },
    standard: {
