...
var CONTENT_PATH = "public/assets/data/knowledge-base.json";
...
function validateKnowledgeBase(input) {
  ...
  TYPES.forEach(function (type) {
    ...
    output[type] = records.slice(0, 300).map(function (record, index) {
      ...
      if (type === "muscles") {
        var imageEvidence = [record.imageUrl, record.imageAlt, record.imageCredit].join(" ");
        if (/cadaver|dissection|specimen|surgical photograph|Gluteus_medius_muscle\.jpg|Anatomist90/i.test(imageEvidence)) errors.push(prefix + " must use an educational illustration instead of a direct anatomy photograph.");
        if (/enko|korean|[\uac00-\ud7af]/i.test(imageEvidence)) errors.push(prefix + " must use an English or language-neutral image without Korean labels.");
        if (/1128_Muscles_of_the_Perineum_Common_to_Men_and_Women/i.test(imageEvidence)) errors.push(prefix + " must use an anatomy illustration instead of a movement chart thumbnail.");
      }
