...
async function createOrder(config, fetchImplementation) {
...
function validateCompletedOrder(order, expectedCaptureId) {
  ...
  if (!order || order.status !== "COMPLETED" || !isOrderId(order.id) || !purchaseUnit ||
      purchaseUnit.custom_id !== PROGRAM.id || !amount || amount.currency_code !== PROGRAM.currency ||
      amount.value !== PROGRAM.amount || !capture || !email) {
    throw new PaypalError("paypal_order_validation_failed", "The completed payment could not be verified.", 409, {
      ...
      programMatches: Boolean(purchaseUnit && purchaseUnit.custom_id === PROGRAM.id),
      amountMatches: Boolean(amount && amount.currency_code === PROGRAM.currency && amount.value === PROGRAM.amount),
