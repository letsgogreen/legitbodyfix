...
var videos = require("../public/assets/data/videos.json");
...
// Every published, playback-ready video with a positive price becomes its own
// purchasable product, in addition to the full-program bundle above. Prices live in
// public/assets/data/videos.json (the same file the admin panel edits) so there is
// one place to change them.
function getVideoProducts() {
  return (Array.isArray(videos) ? videos : [])
    .filter(function (video) {
      return video && video.published !== false && video.streamReady === true &&
        typeof video.streamVideoId === "string" && video.streamVideoId.trim() &&
    ...
    .map(function (video) {
      return {
        id: video.id,
        title: (video.title || video.id) + " — Focused Program",
