...
function getProduct(productId) {
  var id = typeof productId === "string" ? productId.trim() : "";
  if (!id) return null;
...
async function refundCapture(config, captureId, requestId, fetchImplementation) {
  if (!isCaptureId(captureId)) {
    throw new PaypalError("invalid_capture_id", "The PayPal capture is invalid.", 422);
  }

  var idempotencyKey = typeof requestId === "string" ? requestId.trim() : "";
  if (!/^[A-Za-z0-9_-]{10,108}$/.test(idempotencyKey)) {
    throw new PaypalError("invalid_refund_request", "The refund request is invalid.", 422);
  }

  var refund = await paypalRequest(config, "/v2/payments/captures/" + encodeURIComponent(captureId) + "/refund", {
    method: "POST",
    headers: { "PayPal-Request-Id": idempotencyKey },
    body: "{}"
  }, fetchImplementation);

  if (!refund || typeof refund.id !== "string" || !/^[A-Z0-9]{10,40}$/.test(refund.id) ||
      ["COMPLETED", "PENDING"].indexOf(refund.status) === -1) {
    throw new PaypalError("paypal_refund_invalid", "PayPal returned an invalid refund.", 502);
  }

  return {
    id: refund.id,
    status: refund.status,
    amount: refund.amount && typeof refund.amount.value === "string" ? refund.amount.value : "",
    currency: refund.amount && typeof refund.amount.currency_code === "string" ? refund.amount.currency_code : ""
  };
}
