...
var MAX_THUMBNAIL_BYTES = 10 * 1024 * 1024;
...
var ALLOWED_THUMBNAIL_CONTENT_TYPES = {
  "image/jpeg": ".jpg",
  "image/png": ".png",
  "image/webp": ".webp"
};
...
function getUploadConfigIssues(environment) {
  ...
  if (!/^[a-f0-9]{32}$/i.test(accountId)) issues.push("R2_ACCOUNT_ID");
  if (!accessKeyId) issues.push("R2_ACCESS_KEY_ID");
  if (!secretAccessKey) issues.push("R2_SECRET_ACCESS_KEY");
  if (!/^[a-z0-9][a-z0-9.-]{1,61}[a-z0-9]$/.test(bucket)) issues.push("R2_BUCKET");
  if (!publicBaseUrl) issues.push("R2_PUBLIC_BASE_URL");
...
function getUploadConfig(environment) {
  ...
  if (getUploadConfigIssues(env).length) {
    return null;
  }
...
function uploadKind(value) {
  var kind = asText(value || "video").toLowerCase();
  return kind === "video" || kind === "thumbnail" ? kind : "";
}

function allowedContentTypesFor(kind) {
  return kind === "thumbnail" ? ALLOWED_THUMBNAIL_CONTENT_TYPES : ALLOWED_CONTENT_TYPES;
}

function extensionFor(contentType, kind) {
  return allowedContentTypesFor(kind)[String(contentType || "").toLowerCase()] || "";
}

function validateRequest(input) {
  var request = input && typeof input === "object" ? input : {};
  var kind = uploadKind(request.kind);
  var contentType = asText(request.contentType).toLowerCase();
  var fileName = asText(request.fileName);
  var size = Number(request.size);
  var allowedContentTypes = allowedContentTypesFor(kind);
  var maxBytes = kind === "thumbnail" ? MAX_THUMBNAIL_BYTES : MAX_UPLOAD_BYTES;

  if (!kind) {
    throw new R2UploadError("invalid_upload_kind", "The upload kind is invalid.", 400);
  }
  if (!allowedContentTypes[contentType]) {
    throw new R2UploadError(
      kind === "thumbnail" ? "unsupported_thumbnail_type" : "unsupported_video_type",
      kind === "thumbnail" ? "Only JPG, PNG, and WebP thumbnails can be uploaded." : "Only MP4, WebM, and MOV videos can be uploaded.",
      400
    );
  }
  if (!fileName || fileName.length > 180) {
    throw new R2UploadError("invalid_file_name", "The upload file name is invalid.", 400);
  }
  if (!Number.isSafeInteger(size) || size < 1 || size > maxBytes) {
    throw new R2UploadError(
      kind === "thumbnail" ? "invalid_thumbnail_size" : "invalid_file_size",
      kind === "thumbnail" ? "Thumbnails must be between 1 byte and 10 MB." : "Videos must be between 1 byte and 2 GB.",
      400
    );
  }

  return { kind: kind, contentType: contentType, fileName: fileName, size: size };
}
...
function makeObjectKey(request, now, nonce) {
  ...
  var prefix = request.kind === "thumbnail" ? "thumbnails" : "videos";
  return prefix + "/" + date.getUTCFullYear() + "-" + month + "/" +
    date.getTime() + "-" + random + "-" + stem + extensionFor(request.contentType, request.kind);
}
...
function createPresignedUpload(config, input, options) {
  ...
  return {
    ...
    assetUrl: makePublicUrl(config.publicBaseUrl, objectKey),
    objectKey: objectKey,
    kind: request.kind,
...
module.exports = {
  ...
  ALLOWED_THUMBNAIL_CONTENT_TYPES: ALLOWED_THUMBNAIL_CONTENT_TYPES,
  ...
  getUploadConfigIssues: getUploadConfigIssues,
